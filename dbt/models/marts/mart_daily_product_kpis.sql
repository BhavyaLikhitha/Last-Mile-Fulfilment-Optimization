/*
    ...
    Grain: One row per product per day per forecast vintage
    Incremental: merge on [date, product_id, forecast_generated_date]

    ML-written columns (filled by predict_and_writeback.py):
      - demand_forecast          : historical actuals + future forecast
      ...
      - forecast_horizon         : NULL for historical, 30/60/90/180 for future
      - forecast_generated_date  : NULL for historical, date forecast was run for future
                                   Allows multiple forecast vintages to coexist
                                   Power BI filters by this to compare vintages
*/

{{ config(
    materialized='incremental',
    unique_key=['date', 'product_id', 'forecast_generated_date'],
...
select
    ...
    null::date                                                      as forecast_generated_date
