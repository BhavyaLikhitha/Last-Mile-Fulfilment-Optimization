...
def compute_baseline_costs(warehouse_kpis: pd.DataFrame) -> pd.DataFrame:
    ...
    df["stockout_cost_baseline"] = df["stockout_rate"].fillna(0) * df["total_orders"].fillna(0) * avg_order_value * 0.1
...
def compute_optimized_costs(baseline_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["holding_cost_optimized"] = (df["holding_cost_baseline"] * (1 - HOLDING_REDUCTION_RATE)).round(2)

    df["transport_cost_optimized"] = (df["transport_cost_baseline"] * (1 - TRANSPORT_REDUCTION_RATE)).round(2)

    df["ordering_cost_optimized"] = (df["ordering_cost_baseline"] * (1 - ORDERING_REDUCTION_RATE)).round(2)

    df["stockout_cost_optimized"] = (df["stockout_cost_baseline"] * (1 - STOCKOUT_REDUCTION_RATE)).round(2)
    ...
    df["savings_pct"] = (df["savings_amount"] / df["baseline_total_cost"].replace(0, np.nan) * 100).fillna(0).round(2)
...
def compute_allocation_efficiency(orders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_optimal"] = (df["assigned_warehouse_id"] == df["nearest_warehouse_id"]).astype(int)

    agg = (
        df.groupby(["order_date", "assigned_warehouse_id"])
        .agg(total_orders=("order_id", "count"), optimal_orders=("is_optimal", "sum"))
        .reset_index()
        .rename(columns={"order_date": "date", "assigned_warehouse_id": "warehouse_id"})
    )

    agg["allocation_efficiency_pct"] = (
        (agg["optimal_orders"] / agg["total_orders"].replace(0, np.nan) * 100).fillna(0).round(2)
    )
