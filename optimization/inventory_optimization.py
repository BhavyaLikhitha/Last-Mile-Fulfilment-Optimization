...
def compute_optimal_safety_stock(
    avg_daily_demand: float, demand_std: float, lead_time_days: float, lead_time_std: float = 1.0
) -> float:
    """
...
def optimize_reorder_point(
    avg_daily_demand: float, demand_std: float, lead_time_days: float, unit_cost: float, lead_time_std: float = 1.0
) -> dict:
    """
    ...
    if avg_daily_demand <= 0 or lead_time_days <= 0:
        return {"optimal_rop": 0, "safety_stock": 0, "total_cost": 0, "holding_cost": 0, "stockout_cost": 0}
...
def compute_inventory_optimization_summary(
    product_kpis: pd.DataFrame, products: pd.DataFrame, suppliers: pd.DataFrame
) -> pd.DataFrame:
    """
