...
def predict_demand():
    ...
    pull_start = last_scored - pd.Timedelta(days=LOOKBACK_DAYS)
    ...
    print(f"\n  Incremental scoring: {score_from.date()} → {max_date.date()}")
...
def predict_eta():
    ...
    print(f"\n  Incremental ETA scoring: {score_eta_from.date()} → {max_eta_date.date()}")

    # Build chunks dynamically from score_from to max_date
