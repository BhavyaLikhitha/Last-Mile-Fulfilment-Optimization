...
from ml.evaluation.model_metrics import save_comparison
...
from ml.features.demand_features import get_target_column as demand_target
from ml.features.demand_features import train_test_split_temporal as demand_split
...
from ml.features.eta_features import get_target_column as eta_target
from ml.features.eta_features import train_test_split_temporal as eta_split
...
from ml.features.stockout_features import get_target_column as stockout_target
from ml.features.stockout_features import train_test_split_temporal as stockout_split
...
def load_data_from_snowflake(cache_dir: str = "output/cache", use_cache: bool = True, full_refresh: bool = False):
    ...
    for name, config in table_config.items():
        ...
        for batch_df in cur.fetch_pandas_batches():
            ...
            pct = (rows_fetched / total_rows * 100) if total_rows else 0
            elapsed = time.time() - start
            print(f"    Fetched {rows_fetched:,} / {total_rows:,} rows ({pct:.1f}%) — {elapsed:.1f}s elapsed", end="\r")
            batches.append(batch_df)
...
def run_eta_prediction(data: dict) -> dict:
    ...
    X_val = val[available_features].fillna(0)
    y_val = val[target]
    test[available_features].fillna(0)
    test[target]
...
def run_full_pipeline(phases: list = None, full_refresh: bool = False):
    ...
    print("\n  Results saved to ml/results/")
    print("  Next: Write predictions back to Snowflake mart tables")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full-refresh", action="store_true", help="Wipe cache and pull fresh data from Snowflake.")
