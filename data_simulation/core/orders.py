...
v2: Applied year-over-year price inflation, warehouse demand weights,
    category demand weights so charts show real variation.
"""
...
def generate_daily_orders(
    ...
    """
    Generate fact_orders and fact_order_items for a single day.
    Applies YoY price inflation and warehouse/category demand weights.
    """
    batch_id = f"{BATCH_ID_PREFIX}_{current_date.strftime('%Y%m%d')}"
    now      = datetime.combine(current_date, datetime.min.time())
    year     = current_date.year

    # Price inflation multiplier — prices grow 4% per year from 2022 baseline
    price_inflation = get_price_inflation_multiplier(year)

    # Daily order count with seasonality + YoY growth + shock events
    num_orders = get_daily_order_count(DAILY_ORDERS, current_date, rng)
    ...
    # Product arrays — apply price inflation to selling prices
    product_ids      = products_df["product_id"].values
    product_prices   = (products_df["selling_price"].values * price_inflation)
    product_costs    = products_df["cost_price"].values
    product_cats     = products_df["category"].values

    # Build warehouse demand weights for biased customer-to-warehouse assignment
    wh_weights = np.array([
        WAREHOUSE_DEMAND_WEIGHTS.get(wh_id, 1.0) for wh_id in WAREHOUSE_IDS
    ])
    wh_weights = wh_weights / wh_weights.sum()   # normalize to probabilities
    ...
    for i in range(num_orders):
        ...
        if strategy == "nearest":
            ...
            # Bias toward high-demand warehouses for cost optimization
            if rng.random() < 0.3:
                assigned_wh = rng.choice(WAREHOUSE_IDS, p=wh_weights)
            else:
                assigned_wh = nearest_wh
        else:  # load_balanced — use warehouse weights for realistic imbalance
            assigned_wh = rng.choice(WAREHOUSE_IDS, p=wh_weights)
        ...
        # Generate order items — bias product selection by category demand weight
        num_items = int(rng.choice([1, 1, 1, 2, 2, 2, 3, 3, 4, 5]))
