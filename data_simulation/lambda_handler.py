...
def maybe_inject_scd_changes(
    ...
    suppliers_df.loc[s_idx, 'reliability_score'] = (
        suppliers_df.loc[s_idx, 'reliability_score'] + delta
    ...
    print(f"    dim_driver: status rotations applied")
    ...
    if occ_idx:
        ...
        customers_df.loc[upgrade_idx, 'order_frequency_score'] = rng_scd.uniform(0.30, 0.69, size=len(upgrade_idx)).round(2)
...
def generate_days(start_date: date, end_date: date, state_dict: dict = None) -> dict:
    ...
    from data_simulation.core.inventory import initialize_inventory, generate_daily_inventory_snapshot
    ...
    while current_date <= end_date:
        ...
        shipments_df, arriving_df, state.pending_shipments, state.shipment_counter = \
            generate_daily_shipments(
                ...
                state.inventory_state, state.pending_shipments, rng, state.shipment_counter
            )
