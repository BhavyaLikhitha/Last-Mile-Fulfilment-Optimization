...
def generate_base_demand_map(
    ...
    if products_df is not None:
        ...
        for i, (_, row) in enumerate(products_df.iterrows()):
            ...
            demand_map[product_id] = round(
                float(base_demands[i]) * cat_weight * tier_boost, 2
            )
    else:
...
def distribute_demand_across_warehouses(
    ...
    if warehouse_ids is not None:
        ...
        weights = np.array([
            WAREHOUSE_DEMAND_WEIGHTS.get(wh_id, 1.0)
...
def get_category_revenue_multiplier(category: str, year: int) -> float:
    ...
    """
    from config.constants import CATEGORY_GROWTH_MODIFIERS, BASE_YEAR
    base_modifier = CATEGORY_GROWTH_MODIFIERS = {
        "Electronics":   1.04,
        "Health":        1.03,
        "Beauty":        1.02,
        "Sports":        1.01,
