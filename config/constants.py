...
v2: Added year-over-year growth, warehouse demand weights,
    category demand weights, and shock events for realistic variation.
"""
...
# ── Daily Volumes (2022 baseline) ──
# Each subsequent year grows by ANNUAL_GROWTH_RATE
DAILY_ORDERS      = 5000
...
# Orders and revenue grow each year — makes 2025/2026 lines clearly higher than 2022/2023
ANNUAL_GROWTH_RATE = 0.10          # 10% more orders each year
ANNUAL_PRICE_INFLATION = 0.04      # 4% price increase each year (applied in orders.py)
BASE_YEAR = 2022                   # Growth compounds from this year

def get_year_growth_multiplier(year: int) -> float:
    """Returns compounded growth multiplier for a given year vs BASE_YEAR."""
...
def get_price_inflation_multiplier(year: int) -> float:
    """Returns compounded price inflation multiplier for a given year."""
    years_elapsed = max(0, year - BASE_YEAR)
    return (1 + ANNUAL_PRICE_INFLATION) ** years_elapsed

# ── Warehouse Demand Weights ──
# Real fulfillment networks have dominant hubs (NYC, LA) and smaller regional centers
# These weights multiply the base daily order count per warehouse
# LA-West and NYC-East handle 35% more orders than average
# DEN-Mountain and SEA-NW handle 30% fewer
WAREHOUSE_DEMAND_WEIGHTS = {
    "WH-001": 1.35,   # NYC-East      — high population density, dominant hub
    "WH-002": 1.40,   # LA-West       — largest metro, highest volume
    "WH-003": 1.15,   # CHI-Central   — major midwest hub
    "WH-004": 1.10,   # DAL-South     — growing southern market
    "WH-005": 0.75,   # SEA-NW        — smaller regional market
    "WH-006": 0.90,   # MIA-SE        — medium southeast market
    "WH-007": 0.70,   # DEN-Mountain  — smallest market, mountain region
    "WH-008": 0.95,   # ATL-Mid       — mid-atlantic, moderate volume
}

# ── Category Demand Weights ──
# Electronics and Health drive far more revenue than Beauty/Grocery
# These weights multiply base demand when generating order items
CATEGORY_DEMAND_WEIGHTS = {
    "Electronics":    2.20,   # High price + high volume = dominant revenue
    "Home & Garden":  1.40,   # Steady demand, large basket size
    "Health":         1.30,   # Consistent repeat purchases
    "Sports":         1.10,   # Seasonal spikes
    "Apparel":        1.00,   # Baseline
    "Toys":           0.90,   # Seasonal (spikes in Q4 via seasonality)
    "Beauty":         0.80,   # Lower volume
    "Grocery":        0.60,   # Low price per unit, lower revenue contribution
}

# ── Warehouse Distance Factors ──
# Delivery time varies by warehouse geography — coastal warehouses serve wider areas
# These multiply the base delivery time calculation
WAREHOUSE_DISTANCE_FACTORS = {
    "WH-001": 1.20,   # NYC-East      — dense urban, short but traffic-heavy
    "WH-002": 1.35,   # LA-West       — sprawling metro, long distances
    "WH-003": 1.10,   # CHI-Central   — midwest, moderate distances
    "WH-004": 1.25,   # DAL-South     — spread out suburban/rural mix
    "WH-005": 1.45,   # SEA-NW        — large geographic coverage area
    "WH-006": 1.15,   # MIA-SE        — dense coastal corridor
    "WH-007": 1.50,   # DEN-Mountain  — largest coverage area, mountain terrain
    "WH-008": 1.20,   # ATL-Mid       — moderate coverage
}

# ── Shock Events (random disruptions for realistic outliers) ──
# Probability of a disruption event on any given day
SHOCK_EVENT_PROBABILITY = 0.03    # 3% chance per day (~11 events/year)

SHOCK_EVENTS = [
    # Demand spikes (positive shocks)
    {"type": "demand_spike",      "multiplier": 1.8,  "probability": 0.30, "duration_days": 2,
     "description": "Flash sale or viral product — 80% demand surge for 2 days"},
    {"type": "holiday_rush",      "multiplier": 1.5,  "probability": 0.20, "duration_days": 3,
     "description": "Unexpected holiday demand — 50% surge for 3 days"},
    # Supply disruptions (negative shocks)
    {"type": "supplier_delay",    "multiplier": 0.4,  "probability": 0.25, "duration_days": 4,
     "description": "Supplier delay — only 40% of normal shipments arrive for 4 days"},
    {"type": "warehouse_outage",  "multiplier": 0.2,  "probability": 0.15, "duration_days": 2,
     "description": "Warehouse system outage — 80% capacity loss for 2 days"},
    # Moderate disruptions
    {"type": "driver_shortage",   "multiplier": 0.7,  "probability": 0.10, "duration_days": 3,
