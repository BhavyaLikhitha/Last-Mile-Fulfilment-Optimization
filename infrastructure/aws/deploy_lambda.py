...
def get_clients():
    ...
    return (session.client("lambda"), session.client("iam"), session.client("events"), session.client("sts"))
...
def setup_eventbridge(events_client, lambda_client, lambda_arn: str, mode: str = "daily"):
    ...
    for rule in rules:
        ...
        events_client.put_targets(
            Rule=rule["name"], Targets=[{"Id": "1", "Arn": lambda_arn, "Input": json.dumps(rule["payload"])}]
        )
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["daily", "weekly", "disable"],
        default="daily",
        help="EventBridge schedule to enable (default: daily)",
    )
    parser.add_argument("--trigger", action="store_true", help="Manually trigger Lambda after deployment")
    parser.add_argument("--days", type=int, default=1, help="Number of days to generate on manual trigger (default: 1)")
    parser.add_argument("--trigger-only", action="store_true", help="Skip deployment, just trigger existing Lambda")
